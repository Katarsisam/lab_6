...
void key_expansion(vector<unsigned char>& MASTER_KEY, vector<vector<unsigned char>>& ROUND_KEYS) // расширение ключа
{
...
vector<unsigned char> generate_mult_by_number(int x) 
{
    vector<unsigned char> mult_by_number(256);
    for (int i = 0; i < 256; ++i) {
        mult_by_number[i] = galois_multiply(i, x);
    }
...
{
    vector<unsigned char> mult_by_2 = generate_mult_by_number(2); //операции умножения на 2 и 3 к столбцам матрицы данных

    vector<unsigned char> mult_by_3 = generate_mult_by_number(3);
...
{
    vector<unsigned char> mult_by_14 = generate_mult_by_number(14);
    vector<unsigned char> mult_by_9 = generate_mult_by_number(9);
    vector<unsigned char> mult_by_13 = generate_mult_by_number(13);
    vector<unsigned char> mult_by_11 = generate_mult_by_number(11);
...
void block_generate(string& text, vector<vector<vector<unsigned char>>>& BLOCK) { //генерируем изначальное представление сообщения
...
vector<vector<unsigned char>> CIPHER(vector<vector<unsigned char>>& BLOCK, vector<vector<unsigned char>>& ROUND_KEYS) 
{
...
int main() {
    ...
    for (int i = 0; i < (int)ECRYPT.size(); ++i) {
        vector<vector<unsigned char>> res(4, vector<unsigned char>(4, 0));
    ...
    for (int i = 0; i < (int)DECRYPT.size(); i++) {
        vector<vector<unsigned char>>& decrypt = DECRYPT[i];
        for (int j = 0; j < (int)decrypt.size(); j++) {
            for (int k = 0; k < (int)decrypt[j].size(); k++) {
                cout << decrypt[k][j];
            }
        }
    }
